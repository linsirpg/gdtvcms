  ...
  <el-form-item label="标签位置">
    <el-select v-model="Value" placeholder="选择标签位置">
      <el-option    v-for="item in List"  :label="item.memo" :value="item.positionCode" :key='item.recid'></el-option>
		  ...
		  <el-form-item label='标签商品'  style='width:100%;text-align:left;'>
		    <template>
          <textarea name="" id="" cols="30" rows="10" style='width:100%;' v-model="ModelGood.PRODUCT_NO"></textarea>
		    </template>
		  </el-form-item>
		</el-form>
		</div>
    ...
    <!-- <el-tab-pane label="商品"  v-else name="two" >
    ...
    save() {
      ...
      if (this.$route.params.id || this.CMSRECID || this.CMSRECID == "0") {
        ...
        axios
          ...
          axios
            .post("/describe/EditGood", {
              RECID: This.tableData[0].RECID,
              params: This.ModelGood
            })
            .then(function(res) {});
          });
      } else {
        ...
        axios
          ...
          .then(function(res) {
            ...
            This.show1 = false;
            This.ModelGood.RELATION_CODE = res.data.RECID;
            axios
    ...
    if (this.$route.params.id) {
      ...
      axios
        ...
        .then(function(res) {
          ...
          This.tableData = res.data.rows;
          This.ModelGood.PRODUCT_NO = res.data.rows[0].PRODUCT_NO
          This.total1 = res.data.count;
        });
      axios
        .get("describe/GetProDec?RECID=" + this.$route.params.id)
